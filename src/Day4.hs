...
module Day4 (config) where
  
import Lib
import Data.List.Split (chunksOf)
import Data.List (transpose, nub, maximumBy, minimumBy)
import Data.Function (on)

config :: Config BingoGame Int Int
config = Config {title = "Day 4"
                ,input = fmap parseBingo $ parseIntListLines "resources/day4.txt"
                ,part1 = solvePart1
                ,part2 = solvePart2}

type BingoBoard = [[Int]]
data BingoGame = BingoGame {numbers :: [Int], boards :: [BingoBoard]}
data ScoreSheet = ScoreSheet{winningRound :: Int, score :: Int}

parseBingo :: [[Int]] -> BingoGame
parseBingo [] = error "unexpected input"
parseBingo (rawNumbers : rawBoards) = BingoGame
  {numbers= rawNumbers
  ,boards= chunksOf 5. filter (/= []) $ rawBoards
  }
...
determineScore currentRound rowsAndCols currentNumber = ScoreSheet
